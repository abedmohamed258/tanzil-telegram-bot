            ...
            const cleanupPromises = files.map(async (file) => {
                ...
                try {
                    ...
                    if (ageMinutes > maxAgeMinutes) {
                        ...
                        logger.info('🗑️ Old file/directory cleaned', { path: filePath, ageMinutes: ageMinutes.toFixed(1) });
                    }
                } catch (err) {
                    logger.warn('Failed to process file for cleanup', { filePath, error: (err as Error).message });
                }
            });
