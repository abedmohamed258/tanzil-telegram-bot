...
export class RequestQueue {
     ...
     */
    setProcessingCallback(callback: (request: DownloadRequest) => Promise<void>): void {
    ...
    purgeUser(userId: number): void {
        ...
        if (removedCount > 0) {
            logger.info('🗑️ Purged user requests from queue', { userId, count: removedCount });
            this.notifyQueueChange();
        }
    }
     ...
     */
    getStats(): { processing: number; queued: number; memoryStats: { heapUsed: string; heapTotal: string; percentage: string } } {
        return {
            processing: this.processing.size,
            queued: this.queue.length,
